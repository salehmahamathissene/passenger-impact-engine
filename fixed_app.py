"""
Complete working Passenger Impact Engine - FIXED VERSION
"""
...
async def create_company(
    ...
    try:
        ...
        # Map tier to valid database ENUM values: small, mid, large
        tier_map = {
            "small": "small",
            "medium": "mid",      # Map "medium" to "mid"
        ...
        # Build SQL - FIXED: Use proper parameter binding
        sql = text("""
        ...
        for key in list(params.keys()):
            if params[key] is None:
                params[key] = None  # Keep as None, SQLAlchemy handles it
...
async def get_company(
    ...
    try:
        # FIXED: Proper SQL with parameter binding
        sql = text("""
            SELECT 
                id::text, 
                legal_name, 
...
@app.put("/enterprise/companies/{company_id}")
async def update_company(
    ...
    try:
        # Build update SQL
        update_fields = []
        params = {"company_id": company_id, "updated_at": datetime.now()}
        ...
        # Handle tier specially (needs mapping)
        if "tier" in company_data:
            tier_map = {"small": "small", "medium": "mid", "mid": "mid", "large": "large"}
            tier = company_data.get("tier", "small")
            params["tier"] = tier_map.get(tier, "small")
            update_fields.append("tier = :tier")
        
        for json_field, db_field in field_mapping.items():
            if json_field in company_data:
                params[db_field] = company_data[json_field]
...
@app.delete("/enterprise/companies/{company_id}")
async def delete_company(
    ...
    try:
        ...
        check_sql = text("SELECT id FROM enterprise_companies WHERE id = :company_id")
        check_result = db.execute(check_sql, {"company_id": company_id})
        
        if not check_result.fetchone():
            raise HTTPException(status_code=404, detail="Company not found")
        
        # Delete company
        delete_sql = text("DELETE FROM enterprise_companies WHERE id = :company_id")
        result = db.execute(delete_sql, {"company_id": company_id})
        db.commit()
